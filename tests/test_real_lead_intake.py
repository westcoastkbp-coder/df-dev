...
from app.orchestrator import task_worker as task_worker_module
...
def test_real_lead_intake_creates_file_memory_and_logs(monkeypatch, tmp_path: Path) -> None:
    ...
    task_data: dict[str, object] = {
        ...
        "payload": {
            ...
            "summary": "Client wants ADU project, lot 5000 sqft, asking for price",
        },
        "notes": [],
        "history": [],
        "interaction_id": "DF-LEAD-INTAKE-V1",
        "job_id": "DF-LEAD-INTAKE-V1",
        "trace_id": "DF-LEAD-INTAKE-V1",
    }

    def fetch_task(task_id: str) -> dict[str, object] | None:
        return task_data if task_id == "DF-LEAD-INTAKE-V1" else None

    def persist(updated_task: dict[str, object]) -> None:
        task_data.update(updated_task)

    assert queue.enqueue_task("DF-LEAD-INTAKE-V1") is True
    ...
    assert any(
        entry["task_id"] == "DF-LEAD-INTAKE-V1"
    ...
    assert any(
        ...
        and entry["task_id"] == "DF-LEAD-INTAKE-V1"
        and "[MODE]" in entry["details"].get("message", "")
        for entry in system_log
    )
    assert any(
        "completed WRITE_FILE -> runtime/out/leads/lead_001.txt"
